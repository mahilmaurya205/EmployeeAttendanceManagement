          ...
          {employees.map(emp => (
            <div key={emp._id} className="card hover:border-slate-600 transition-all group cursor-pointer"
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="badge badge-info text-xs shrink-0 max-w-28 truncate" title={emp.department}>
                  {emp.department || 'N/A'}
                </span>
              </div>
              ...
              <div className="space-y-1 text-xs text-slate-500">
                ...
                <div className="flex items-center gap-1.5">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-3.314 0-6 1.343-6 3s2.686 3 6 3 6-1.343 6-3-2.686-3-6-3zm0 0V5m0 9v5m-7-7H3m18 0h-2" /></svg>
                  Basic Salary: ₹{Number(emp.basicSalary || 0).toLocaleString('en-IN')}
                </div>
              </div>
